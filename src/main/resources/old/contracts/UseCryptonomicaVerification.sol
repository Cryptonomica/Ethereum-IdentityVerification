...
contract UseCryptonomicaVerification {
    ...
    function checkUsCitizen(address userAddress) returns (bool){
        // see:
    ...
    // 0
    function requestFingerprint(address userAddress) returns (bytes32){
        fingerprint[userAddress] = cryptonomicaVerification.fingerprint(userAddress);
        return fingerprint[userAddress];
    }

    // 1
    function requestKeyCertificateValidUntil(address userAddress) returns (uint){
    ...
    // 2
    function requestUserData(address userAddress){
